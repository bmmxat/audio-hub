...
- **修复跨设备应用音量控制**：应用输出被指定到非默认扬声器后，音量滑块与静音按钮
  会操作实际路由端点，不再误改默认扬声器上残留的同应用会话。
- **改进多端点会话识别**：同一应用在多个输出端点留下会话时，优先使用正在发声的
  会话；状态相同时再使用默认输出端点，降低路由切换后的旧会话干扰。
- **修复未聚焦自动静音的多设备行为**：自动静音及状态恢复会记录并操作应用当时的
  实际输出端点；自动静音期间切换默认输出或应用指定输出时，会按 Windows 路由重新
  绑定目标端点。旧 Profile 和旧版恢复记录也会先解析当前会话端点，不再遍历修改
  同一 PID 在所有扬声器上的会话。
- **隔离临时静音与设备音量快照**：“音量随扬声器”会保存未聚焦静音前的原始静音
  状态；恢复快照时只恢复这类应用的音量，不会取消临时静音。设备或会话变化后会
  立即重新绑定未聚焦静音；迁移时先静音新端点，并在旧会话停止后才恢复旧端点，
  避免两项功能相互覆盖或在设备切换过程中短暂漏音。
